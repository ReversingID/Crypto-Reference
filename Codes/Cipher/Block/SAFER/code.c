...
Compile:
    ...
    $ cl /I. main.c mode.c SAFER/code.c

    (gcc, from Codes/Cipher/Block/)
    $ gcc -I. -o test main.c mode.c SAFER/code.c

    Modes of operation are in mode.c (not in this file).

Assemble:
    (gcc)
    $ gcc -m32 -S -masm=intel -o SAFER/code.asm SAFER/code.c

    (msvc)
    $ cl /c /FaSAFER/code.asm SAFER/code.c
*/
#include <stdint.h>
#include <string.h>
...
{
    key_setup((safer_t *)ctx, (uint8_t *)key, KEYSIZEB);
}

void
cipher_encrypt_block(uint8_t *ctx, uint8_t *block)
{
    block_encrypt((safer_t *)ctx, block);
}

void
cipher_decrypt_block(uint8_t *ctx, uint8_t *block)
{
    block_decrypt((safer_t *)ctx, block);
}

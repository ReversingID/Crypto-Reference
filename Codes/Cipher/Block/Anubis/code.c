...
Compile:
    ...
    $ cl /I. main.c mode.c Anubis/code.c

    (gcc, from Codes/Cipher/Block/)
    $ gcc -I. -o test main.c mode.c Anubis/code.c

    Modes of operation are in mode.c (not in this file).

Assemble:
    (gcc)
    $ gcc -m32 -S -masm=intel -o Anubis/code.asm Anubis/code.c

    (msvc)
    $ cl /c /FaAnubis/code.asm Anubis/code.c
*/
#include <stdint.h>
#include <string.h>
...
{
    key_setup((anubis_t *)ctx, (uint8_t *)key, KEYSIZE);
}

void
cipher_encrypt_block(uint8_t *ctx, uint8_t *block)
{
    block_encrypt((anubis_t *)ctx, block);
}

void
cipher_decrypt_block(uint8_t *ctx, uint8_t *block)
{
    block_decrypt((anubis_t *)ctx, block);
}

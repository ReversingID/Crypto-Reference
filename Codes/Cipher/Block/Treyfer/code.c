...
Compile:
    ...
    $ cl /I. main.c mode.c Treyfer/code.c

    (gcc, from Codes/Cipher/Block/)
    $ gcc -I. -o test main.c mode.c Treyfer/code.c

    Modes of operation are in mode.c (not in this file).

Assemble:
    (gcc)
    $ gcc -m32 -S -masm=intel -o Treyfer/code.asm Treyfer/code.c

    (msvc)
    $ cl /c /FaTreyfer/code.asm Treyfer/code.c
*/
...
void cipher_encrypt_block(uint8_t *ctx, uint8_t *block) { block_encrypt(block, ctx); }
void cipher_decrypt_block(uint8_t *ctx, uint8_t *block) { block_decrypt(block, ctx); }

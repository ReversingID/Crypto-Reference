/*
    MISTY1 by Mitsubishi Electric (Mitsuru Matsui et al.)
    Archive of Reversing.ID
    Block Cipher

Compile:
    (msvc, from Codes/Cipher/Block/)
    $ cl /I. main.c mode.c MISTY1/code.c

    (gcc, from Codes/Cipher/Block/)
    $ gcc -I. -o test main.c mode.c MISTY1/code.c

    Modes of operation are in mode.c (not in this file).

Assemble:
    (gcc)
    $ gcc -m32 -S -masm=intel -o MISTY1/code.asm MISTY1/code.c

    (msvc)
    $ cl /c /FaMISTY1/code.asm MISTY1/code.c

Note:
    MISTY1 (64-bit block, 8 rounds, 128-bit key). RFC 2994.
*/
#include <stdint.h>
#include <string.h>
#include "../byteorder.h"
...
void block_encrypt(misty1_t *config, uint8_t val[BLOCKSIZEB]);
void block_decrypt(misty1_t *config, uint8_t val[BLOCKSIZEB]);
void key_setup(misty1_t *config, const uint8_t *key);

static uint16_t fi(uint16_t fi_in, uint16_t fi_key);
static uint32_t fo(uint32_t fo_in, unsigned k, const uint16_t *ek);
static uint32_t fl(uint32_t fl_in, unsigned k, const uint16_t *ek);
static uint32_t fl_inv(uint32_t fl_in, unsigned k, const uint16_t *ek);


/* *************************** HELPER FUNCTIONS *************************** */
static uint16_t
fi(uint16_t fi_in, uint16_t fi_key)
{
    uint16_t d9;
    uint16_t d7;

    d9 = (uint16_t)(fi_in >> 7);
    d7 = (uint16_t)(fi_in & 0x7f);
    d9 = (uint16_t)(S9[d9] ^ d7);
    d7 = (uint16_t)(S7[d7] ^ (d9 & 0x7F));
    d7 = (uint16_t)(d7 & 0x7f);
    d7 = (uint16_t)(d7 ^ (fi_key >> 9));
    d9 = (uint16_t)(d9 ^ (fi_key & 0x1ff));
    d9 = (uint16_t)(S9[d9] ^ d7);
    return (uint16_t)((d7 << 9) | d9);
}

static uint32_t
fo(uint32_t fo_in, unsigned k, const uint16_t *ek)
{
    uint16_t t0;
    uint16_t t1;

    t0 = (uint16_t)(fo_in >> 16);
    t1 = (uint16_t)(fo_in & 0xffff);
    t0 = (uint16_t)(t0 ^ ek[k]);
    t0 = fi(t0, ek[(k + 5) % 8 + 8]);
    t0 = (uint16_t)(t0 ^ t1);
    t1 = (uint16_t)(t1 ^ ek[(k + 2) % 8]);
    t1 = fi(t1, ek[(k + 1) % 8 + 8]);
    t1 = (uint16_t)(t1 ^ t0);
    t0 = (uint16_t)(t0 ^ ek[(k + 7) % 8]);
    t0 = fi(t0, ek[(k + 3) % 8 + 8]);
    t0 = (uint16_t)(t0 ^ t1);
    t1 = (uint16_t)(t1 ^ ek[(k + 4) % 8]);
    return ((uint32_t)t1 << 16) | t0;
}

static uint32_t
fl(uint32_t fl_in, unsigned k, const uint16_t *ek)
{
    uint16_t d0;
    uint16_t d1;

    d0 = (uint16_t)(fl_in >> 16);
    d1 = (uint16_t)(fl_in & 0xffff);
    if ((k & 1u) == 0)
    {
        d1 = (uint16_t)(d1 ^ (d0 & ek[k / 2]));
        d0 = (uint16_t)(d0 ^ (d1 | ek[(k / 2 + 6) % 8 + 8]));
    }
    else
    {
        d1 = (uint16_t)(d1 ^ (d0 & ek[((k - 1) / 2 + 2) % 8 + 8]));
        d0 = (uint16_t)(d0 ^ (d1 | ek[((k - 1) / 2 + 4) % 8]));
    }
    return ((uint32_t)d0 << 16) | d1;
}

static uint32_t
fl_inv(uint32_t fl_in, unsigned k, const uint16_t *ek)
{
    uint16_t d0;
    uint16_t d1;
...
void
key_setup(misty1_t *config, const uint8_t *key)
{
    unsigned i;

    for (i = 0; i < 8; i++)
        config->ek[i] = (uint16_t)((key[i * 2] << 8) | key[i * 2 + 1]);

    for (i = 0; i < 8; i++)
    {
        config->ek[i + 8]  = fi(config->ek[i], config->ek[(i + 1) % 8]);
        config->ek[i + 16] = (uint16_t)(config->ek[i + 8] & 0x1ff);
        config->ek[i + 24] = (uint16_t)(config->ek[i + 8] >> 9);
    }
}

void
block_encrypt(misty1_t *config, uint8_t val[BLOCKSIZEB])
{
    uint32_t d0;
    uint32_t d1;
    const uint16_t *ek = config->ek;
...
void
block_decrypt(misty1_t *config, uint8_t val[BLOCKSIZEB])
{
    uint32_t d0;
    uint32_t d1;
    const uint16_t *ek = config->ek;

    /* C = (D1 << 32) | D0; low half is at val + 4 */
    d1 = load32_be(val);
    d0 = load32_be(val + 4);

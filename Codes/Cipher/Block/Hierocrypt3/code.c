...
Compile:
    ...
    $ cl /I. main.c mode.c Hierocrypt3/code.c

    (gcc, from Codes/Cipher/Block/)
    $ gcc -I. -o test main.c mode.c Hierocrypt3/code.c

    Modes of operation are in mode.c (not in this file).

Assemble:
    (gcc)
    $ gcc -m32 -S -masm=intel -o Hierocrypt3/code.asm Hierocrypt3/code.c

    (msvc)
    $ cl /c /FaHierocrypt3/code.asm Hierocrypt3/code.c
*/
#include <stdint.h>
#include <string.h>
...
block_encrypt(hc3_t * config, uint8_t val[BLOCKSIZEB])
{
    ...
    // ubah val menjadi representasi matriks
    for (r = i = 0; i < 4; i++)
        for (j = 0; j < 4; j++, r++)
            t[i][j] = val[r];
...
/* menukarkan kunci dari block L ke block R */
void 
swap_key (uint8_t * L, uint8_t * R)
{
    uint8_t  t;
    uint32_t i;
    for (i = 0; i < 8; i++)
...
/*
    Mendapatkan derajat sebuah polinomial.
    Polinomial direpresentasikan sebagai bit.
*/
int32_t  
polynom32_degree   (uint32_t a)
{
    int32_t n = -1;
    for (; a; a >>= 1)
        n ++;
...
/*
    Melakukan perkalian polinomial.
    Polinomial direpresentasikan sebagai bit.
*/
uint32_t 
polynom32_multiply (uint32_t a, uint32_t b)
{
    uint32_t c = 0;
...
/*
    Operasi mode pada polinomial
*/
uint32_t 
polynom32_mod      (uint32_t a, uint32_t b)
{
...
void 
mdsh_multiply (uint8_t * dst, uint8_t * src, uint32_t x)
{
    uint32_t i;
    uint8_t  u[4];

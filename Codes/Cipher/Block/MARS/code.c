...
Compile:
    ...
    $ cl /I. main.c mode.c MARS/code.c

    (gcc, from Codes/Cipher/Block/)
    $ gcc -I. -o test main.c mode.c MARS/code.c

    Modes of operation are in mode.c (not in this file).

Assemble:
    (gcc)
    $ gcc -m32 -S -masm=intel -o MARS/code.asm MARS/code.c

    (msvc)
    $ cl /c /FaMARS/code.asm MARS/code.c
*/
#include <stdint.h>
#include <string.h>
...
void key_setup(mars_t * config, uint8_t * secret, uint32_t bits);
...
void mars_crypt(uint8_t val[BLOCKSIZEB], const uint32_t rkeys[MAX_ROUNDS + 1][4], int R);

uint32_t gen_mask(uint32_t x);

/* ************************ CRYPTOGRAPHY ALGORITHM ************************ */
/* 
    Enkripsi sebuah block dengan MARS. 
    Pastikan konfigurasi telah dilakukan dengan memanggil key_setup()
*/
void 
block_encrypt(mars_t * config, uint8_t val[BLOCKSIZEB])
{
    uint32_t   a, b, c, d;
...
/* 
    Dekripsi sebuah block dengan MARS. 
    Pastikan konfigurasi telah dilakukan dengan memanggil key_setup()
*/
void 
block_decrypt(mars_t * config, uint8_t val[BLOCKSIZEB])
{
    uint32_t   a, b, c, d;
    uint32_t * p_val = (uint32_t *) val;
...
/*
    length = panjang key dalam byte
        (16, 24, 32) untuk representasi (128, 192, 256) bit
*/
void key_setup(mars_t * config, uint8_t * key, uint32_t length)
{
    uint32_t   i, j, m, w;
...
{
    key_setup((mars_t *)ctx, (uint8_t *)key, KEYSIZEB);
}

void
cipher_encrypt_block(uint8_t *ctx, uint8_t *block)
{
    block_encrypt((mars_t *)ctx, block);
}

void
cipher_decrypt_block(uint8_t *ctx, uint8_t *block)
{
    block_decrypt((mars_t *)ctx, block);
}

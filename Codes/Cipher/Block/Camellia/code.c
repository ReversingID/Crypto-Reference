...
Compile:
    ...
    $ cl /I. main.c mode.c Camellia/code.c

    (gcc, from Codes/Cipher/Block/)
    $ gcc -I. -o test main.c mode.c Camellia/code.c

    Modes of operation are in mode.c (not in this file).

Assemble:
    (gcc)
    $ gcc -m32 -S -masm=intel -o Camellia/code.asm Camellia/code.c

    (msvc)
    $ cl /c /FaCamellia/code.asm Camellia/code.c
*/
#include <stdint.h>
#include <memory.h>
...
{
    key_setup((camellia_t *)ctx, (uint8_t *)key, KEYSIZE);
}

void
cipher_encrypt_block(uint8_t *ctx, uint8_t *block)
{
    block_encrypt((camellia_t *)ctx, block);
}

void
cipher_decrypt_block(uint8_t *ctx, uint8_t *block)
{
    block_decrypt((camellia_t *)ctx, block);
}

...
Compile:
    ...
    $ cl /I. main.c mode.c CIPHERUNICORN-A/code.c

    (gcc, from Codes/Cipher/Block/)
    $ gcc -I. -o test main.c mode.c CIPHERUNICORN-A/code.c

    Modes of operation are in mode.c (not in this file).

Assemble:
    (gcc)
    $ gcc -m32 -S -masm=intel -o CIPHERUNICORN-A/code.asm CIPHERUNICORN-A/code.c

    (msvc)
    $ cl /c /FaCIPHERUNICORN-A/code.asm CIPHERUNICORN-A/code.c
*/
#include <stdint.h>
#include <memory.h>
...
{
    key_setup((unicorn_t *)ctx, (uint8_t *)key);
}

void
cipher_encrypt_block(uint8_t *ctx, uint8_t *block)
{
    block_encrypt((unicorn_t *)ctx, block);
}

void
cipher_decrypt_block(uint8_t *ctx, uint8_t *block)
{
    block_decrypt((unicorn_t *)ctx, block);
}

...
Note:
    SNOW 2.0 with 128-bit key and 128-bit IV.
    Spec also defines 256-bit keys; this build uses 128-bit only.
*/
...
/* ************************ CONFIGURATION & SEED ************************ */
#define SNOW_BYTE(n, w)   (((w) >> ((n) * 8)) & 0xffU)
#define SNOW_AINV_MUL(w)  (((w) >> 8) ^ snow_alphainv_mul[(w) & 0xffU])
#define SNOW_A_MUL(w)     (((w) << 8) ^ snow_alpha_mul[(w) >> 24])

static const uint32_t snow_alpha_mul[256] = {
    ...
    0x667BFF0A,0x87E43019,0x0DEC882C,0xEC73073F,0xB0FC9146,0x51635E55,0xDB6BA660,0x3AF46973,
    0x63DC2392,0x8243EC81,0x084B14B4,0xE9D4DBA7,0xB55B4DDE,0x54C482CD,0xDECC7AF8,0x3F53B5EB
};

static const uint32_t snow_alphainv_mul[256] = {
...
static const uint32_t snow_T0[256] = {
    ...
    0x3be0e0db,0x56323264,0x4e3a3a74,0x1e0a0a14,0xdb494992,0x0a6060c0,0x6c242448,0xe45c5cb8,
...
static const uint32_t snow_T1[256] = {
    ...
    0xe0e0db3b,0x32326456,0x3a3a744e,0x0a0a141e,0x494992db,0x6060c0a0,0x2424486c,0x5c5cb8e4,
...
static const uint32_t snow_T2[256] = {
...
static const uint32_t snow_T3[256] = {
    ...
    0x038f8c8c,0x59f8a1a1,0x09898089,0x1a170d0d,0x65dabfbf,0xd731e6e6,0x84c64242,0xd0b86868,
    0x82c34141,0x29b09999,0x5a772d2d,0x1e110f0f,0x7bcbb0b0,0xa8fc5454,0x6dd6bbbb,0x2c3a1616
};

typedef struct snow_ctx_t {
    uint32_t s0, s1, s2, s3, s4, s5, s6, s7;
    uint32_t s8, s9, s10, s11, s12, s13, s14, s15;
    uint32_t r1;
    uint32_t r2;
} snow_ctx_t;
...
static void stream_crypt(snow_ctx_t *ctx, uint8_t *data, size_t length);
static void key_setup(snow_ctx_t *ctx, const uint8_t *key, const uint8_t *iv);

static void fsm_update(snow_ctx_t *ctx, uint32_t fsmtmp);
static void init_clock(snow_ctx_t *ctx);
static void keystream_block(snow_ctx_t *ctx, uint32_t *keystream_block);


/* ************************ CRYPTOGRAPHY ALGORITHM ************************ */
static void
stream_crypt(snow_ctx_t *ctx, uint8_t *data, size_t length)
{
    size_t offset = 0;
    ...
    while (offset < length)
    {
        ...
        keystream_block(ctx, keystream);
        chunk = length - offset;
        if (chunk > 64)
            chunk = 64;

        for (j = 0; j < chunk; j++)
            data[offset + j] ^= (uint8_t)(keystream[j / 4] >> (24 - (uint32_t)(j % 4) * 8));
...
static void
key_setup(snow_ctx_t *ctx, const uint8_t *key, const uint8_t *iv)
{
    ctx->s15 = ((uint32_t)key[0] << 24) | ((uint32_t)key[1] << 16) |
               ((uint32_t)key[2] << 8) | (uint32_t)key[3];
    ctx->s14 = ((uint32_t)key[4] << 24) | ((uint32_t)key[5] << 16) |
               ((uint32_t)key[6] << 8) | (uint32_t)key[7];
    ctx->s13 = ((uint32_t)key[8] << 24) | ((uint32_t)key[9] << 16) |
               ((uint32_t)key[10] << 8) | (uint32_t)key[11];
    ctx->s12 = ((uint32_t)key[12] << 24) | ((uint32_t)key[13] << 16) |
    ...
    ctx->s15 ^= ((uint32_t)iv[0] << 24) | ((uint32_t)iv[1] << 16) |
                ((uint32_t)iv[2] << 8) | (uint32_t)iv[3];
    ctx->s12 ^= ((uint32_t)iv[4] << 24) | ((uint32_t)iv[5] << 16) |
                ((uint32_t)iv[6] << 8) | (uint32_t)iv[7];
    ctx->s10 ^= ((uint32_t)iv[8] << 24) | ((uint32_t)iv[9] << 16) |
                ((uint32_t)iv[10] << 8) | (uint32_t)iv[11];
    ctx->s9  ^= ((uint32_t)iv[12] << 24) | ((uint32_t)iv[13] << 16) |
...
static void
fsm_update(snow_ctx_t *ctx, uint32_t fsmtmp)
{
    ctx->r2 = snow_T0[SNOW_BYTE(0, ctx->r1)] ^ snow_T1[SNOW_BYTE(1, ctx->r1)] ^
              snow_T2[SNOW_BYTE(2, ctx->r1)] ^ snow_T3[SNOW_BYTE(3, ctx->r1)];
    ctx->r1 = fsmtmp;
}

static void
init_clock(snow_ctx_t *ctx)
{
    uint32_t outfrom_fsm;
    uint32_t fsmtmp;
    int i;
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        ctx->s0 = SNOW_A_MUL(ctx->s0) ^ ctx->s2 ^ SNOW_AINV_MUL(ctx->s11) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s5;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s0) ^ ctx->r2;
        ctx->s1 = SNOW_A_MUL(ctx->s1) ^ ctx->s3 ^ SNOW_AINV_MUL(ctx->s12) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s6;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s1) ^ ctx->r2;
        ctx->s2 = SNOW_A_MUL(ctx->s2) ^ ctx->s4 ^ SNOW_AINV_MUL(ctx->s13) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s7;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s2) ^ ctx->r2;
        ctx->s3 = SNOW_A_MUL(ctx->s3) ^ ctx->s5 ^ SNOW_AINV_MUL(ctx->s14) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s8;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s3) ^ ctx->r2;
        ctx->s4 = SNOW_A_MUL(ctx->s4) ^ ctx->s6 ^ SNOW_AINV_MUL(ctx->s15) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s9;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s4) ^ ctx->r2;
        ctx->s5 = SNOW_A_MUL(ctx->s5) ^ ctx->s7 ^ SNOW_AINV_MUL(ctx->s0) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s10;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s5) ^ ctx->r2;
        ctx->s6 = SNOW_A_MUL(ctx->s6) ^ ctx->s8 ^ SNOW_AINV_MUL(ctx->s1) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s11;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s6) ^ ctx->r2;
        ctx->s7 = SNOW_A_MUL(ctx->s7) ^ ctx->s9 ^ SNOW_AINV_MUL(ctx->s2) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s12;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s7) ^ ctx->r2;
        ctx->s8 = SNOW_A_MUL(ctx->s8) ^ ctx->s10 ^ SNOW_AINV_MUL(ctx->s3) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s13;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s8) ^ ctx->r2;
        ctx->s9 = SNOW_A_MUL(ctx->s9) ^ ctx->s11 ^ SNOW_AINV_MUL(ctx->s4) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s14;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s9) ^ ctx->r2;
        ctx->s10 = SNOW_A_MUL(ctx->s10) ^ ctx->s12 ^ SNOW_AINV_MUL(ctx->s5) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s15;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s10) ^ ctx->r2;
        ctx->s11 = SNOW_A_MUL(ctx->s11) ^ ctx->s13 ^ SNOW_AINV_MUL(ctx->s6) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s0;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s11) ^ ctx->r2;
        ctx->s12 = SNOW_A_MUL(ctx->s12) ^ ctx->s14 ^ SNOW_AINV_MUL(ctx->s7) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s1;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s12) ^ ctx->r2;
        ctx->s13 = SNOW_A_MUL(ctx->s13) ^ ctx->s15 ^ SNOW_AINV_MUL(ctx->s8) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s2;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s13) ^ ctx->r2;
        ctx->s14 = SNOW_A_MUL(ctx->s14) ^ ctx->s0 ^ SNOW_AINV_MUL(ctx->s9) ^ outfrom_fsm;
        fsmtmp = ctx->r2 + ctx->s3;
        fsm_update(ctx, fsmtmp);

        outfrom_fsm = (ctx->r1 + ctx->s14) ^ ctx->r2;
        ctx->s15 = SNOW_A_MUL(ctx->s15) ^ ctx->s1 ^ SNOW_AINV_MUL(ctx->s10) ^ outfrom_fsm;
...
static void
keystream_block(snow_ctx_t *ctx, uint32_t *keystream_block)
{
    uint32_t fsmtmp;

    ctx->s0 = SNOW_A_MUL(ctx->s0) ^ ctx->s2 ^ SNOW_AINV_MUL(ctx->s11);
    fsmtmp = ctx->r2 + ctx->s5;
    fsm_update(ctx, fsmtmp);
    keystream_block[0] = (ctx->r1 + ctx->s0) ^ ctx->r2 ^ ctx->s1;

    ctx->s1 = SNOW_A_MUL(ctx->s1) ^ ctx->s3 ^ SNOW_AINV_MUL(ctx->s12);
    fsmtmp = ctx->r2 + ctx->s6;
    fsm_update(ctx, fsmtmp);
    keystream_block[1] = (ctx->r1 + ctx->s1) ^ ctx->r2 ^ ctx->s2;

    ctx->s2 = SNOW_A_MUL(ctx->s2) ^ ctx->s4 ^ SNOW_AINV_MUL(ctx->s13);
    fsmtmp = ctx->r2 + ctx->s7;
    fsm_update(ctx, fsmtmp);
    keystream_block[2] = (ctx->r1 + ctx->s2) ^ ctx->r2 ^ ctx->s3;

    ctx->s3 = SNOW_A_MUL(ctx->s3) ^ ctx->s5 ^ SNOW_AINV_MUL(ctx->s14);
    fsmtmp = ctx->r2 + ctx->s8;
    fsm_update(ctx, fsmtmp);
    keystream_block[3] = (ctx->r1 + ctx->s3) ^ ctx->r2 ^ ctx->s4;

    ctx->s4 = SNOW_A_MUL(ctx->s4) ^ ctx->s6 ^ SNOW_AINV_MUL(ctx->s15);
    fsmtmp = ctx->r2 + ctx->s9;
    fsm_update(ctx, fsmtmp);
    keystream_block[4] = (ctx->r1 + ctx->s4) ^ ctx->r2 ^ ctx->s5;

    ctx->s5 = SNOW_A_MUL(ctx->s5) ^ ctx->s7 ^ SNOW_AINV_MUL(ctx->s0);
    fsmtmp = ctx->r2 + ctx->s10;
    fsm_update(ctx, fsmtmp);
    keystream_block[5] = (ctx->r1 + ctx->s5) ^ ctx->r2 ^ ctx->s6;

    ctx->s6 = SNOW_A_MUL(ctx->s6) ^ ctx->s8 ^ SNOW_AINV_MUL(ctx->s1);
    fsmtmp = ctx->r2 + ctx->s11;
    fsm_update(ctx, fsmtmp);
    keystream_block[6] = (ctx->r1 + ctx->s6) ^ ctx->r2 ^ ctx->s7;

    ctx->s7 = SNOW_A_MUL(ctx->s7) ^ ctx->s9 ^ SNOW_AINV_MUL(ctx->s2);
    fsmtmp = ctx->r2 + ctx->s12;
    fsm_update(ctx, fsmtmp);
    keystream_block[7] = (ctx->r1 + ctx->s7) ^ ctx->r2 ^ ctx->s8;

    ctx->s8 = SNOW_A_MUL(ctx->s8) ^ ctx->s10 ^ SNOW_AINV_MUL(ctx->s3);
    fsmtmp = ctx->r2 + ctx->s13;
    fsm_update(ctx, fsmtmp);
    keystream_block[8] = (ctx->r1 + ctx->s8) ^ ctx->r2 ^ ctx->s9;

    ctx->s9 = SNOW_A_MUL(ctx->s9) ^ ctx->s11 ^ SNOW_AINV_MUL(ctx->s4);
    fsmtmp = ctx->r2 + ctx->s14;
    fsm_update(ctx, fsmtmp);
    keystream_block[9] = (ctx->r1 + ctx->s9) ^ ctx->r2 ^ ctx->s10;

    ctx->s10 = SNOW_A_MUL(ctx->s10) ^ ctx->s12 ^ SNOW_AINV_MUL(ctx->s5);
    fsmtmp = ctx->r2 + ctx->s15;
    fsm_update(ctx, fsmtmp);
    keystream_block[10] = (ctx->r1 + ctx->s10) ^ ctx->r2 ^ ctx->s11;

    ctx->s11 = SNOW_A_MUL(ctx->s11) ^ ctx->s13 ^ SNOW_AINV_MUL(ctx->s6);
    fsmtmp = ctx->r2 + ctx->s0;
    fsm_update(ctx, fsmtmp);
    keystream_block[11] = (ctx->r1 + ctx->s11) ^ ctx->r2 ^ ctx->s12;

    ctx->s12 = SNOW_A_MUL(ctx->s12) ^ ctx->s14 ^ SNOW_AINV_MUL(ctx->s7);
    fsmtmp = ctx->r2 + ctx->s1;
    fsm_update(ctx, fsmtmp);
    keystream_block[12] = (ctx->r1 + ctx->s12) ^ ctx->r2 ^ ctx->s13;

    ctx->s13 = SNOW_A_MUL(ctx->s13) ^ ctx->s15 ^ SNOW_AINV_MUL(ctx->s8);
    fsmtmp = ctx->r2 + ctx->s2;
    fsm_update(ctx, fsmtmp);
    keystream_block[13] = (ctx->r1 + ctx->s13) ^ ctx->r2 ^ ctx->s14;

    ctx->s14 = SNOW_A_MUL(ctx->s14) ^ ctx->s0 ^ SNOW_AINV_MUL(ctx->s9);
    fsmtmp = ctx->r2 + ctx->s3;
    fsm_update(ctx, fsmtmp);
    keystream_block[14] = (ctx->r1 + ctx->s14) ^ ctx->r2 ^ ctx->s15;

    ctx->s15 = SNOW_A_MUL(ctx->s15) ^ ctx->s1 ^ SNOW_AINV_MUL(ctx->s10);
    fsmtmp = ctx->r2 + ctx->s4;
    fsm_update(ctx, fsmtmp);
 ...
 *
 * Variant : SNOW 2.0, 128-bit key + 128-bit IV (not SNOW-3G).
 *
 * STREAM_KEY_BYTES     = 32 — key[0..15], 128-bit key (big-endian words);
 *                             key[16..31] ignored.
